...
class Ball {
  ...
  do() {
    ...
    if (
      this.y < wallWidth + this.size / 2 ||
      this.y > height - wallWidth - this.size / 2
    ) {
      this.speedY *= -1;
    }
    ...
    if (
      this.x - this.size / 2 < paddleWidth &&
      this.y > paddles[0].y - paddleHeight / 2 &&
      this.y < paddles[0].y + paddleHeight / 2
    ) {
    ...
    if (
      this.x + this.size / 2 > width - paddleWidth &&
      this.y > paddles[1].y - paddleHeight / 2 &&
      this.y < paddles[1].y + paddleHeight / 2
    ) {
...
function drawWalls() {
  //this function draws the walls every frame
  rectMode(CORNER);
  fill(215);
